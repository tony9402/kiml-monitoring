...
from csms.sdk.utils.flat_client import FlatApiClient, CosmosError
...
def send_submit(workspace: Workspace, experiment, src_dir: str, command: str, image: Image,
            resource_flavor: str, name: str = None, datasets: List[Dataset] = None, replica: int = 1,
            env_vars: Dict[str, str] = {}, description: str = None, tags: Dict[str, str] = {}):
    ...
    domain_id, project_id, workspace_id = config.domain_id, config.project_id, workspace.id
    ...
    if errs:
        raise CosmosError(f"Run.create() fails. invalid name: {name}, errs: {errs}")

    if not image.path:
        raise CosmosError(f"Run.create() fails. missing path of run image: {image.name}."
    ...
    if tags:
        for tag_key in tags:
            ...
            if errs:
                raise CosmosError(f"create run fail. invalid tag key:{tag_key}, errs: {errs}")

    fname = uuid4.hex()
    fpath = f"{fname}.tar.gz"
    make_tarfile(fname, src_dir)
    with open(fpath, 'rb') as f:
        code_model = code_api_instance.api_v1_domains_domain_id_projects_project_id_workspaces_workspace_id_codes_post(
            domain_id, project_id, workspace_id, upload=f)
    os.remove(fpath)

    r = RunRequest(name=name,
